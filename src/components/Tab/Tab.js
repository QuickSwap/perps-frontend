...
export default function Tab(props) {
  ...
  return (
    <div className={cx("Tab", type, className)}>
      {options.map((opt) => {
        const label = optionLabels && optionLabels[opt] ? optionLabels[opt] : opt.html ?? opt;
        return (
          <div className={cx("Tab-option", "muted", { active: (opt.value ?? opt) === option })} onClick={() => onClick(opt)} key={opt}>
